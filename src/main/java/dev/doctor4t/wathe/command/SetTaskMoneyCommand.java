...
public class SetTaskMoneyCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(
                CommandManager.literal("wathe:setTaskMoney")
        ...
        return Wathe.executeSupporterCommand(source,
                () -> {
                    for (Entity target : targets) {
                        PlayerShopComponent.KEY.get(target).setCurrencyAmount(EconomyApi.TASK_MONEY, amount);
                    }
                }
        );
    }
